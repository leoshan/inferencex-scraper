...
数据存储：
    - JSON 原始数据: data/raw/json/inferencex/
    - CSV 文件: data/processed/csv/inferencex/
"""
...
class APIDataCollector:
    ...
    def save_json_file(self, data: List[Dict], model_id: str,
        ...
        display_name = self.get_model_display_name(model_id)
        filename = f"{response_index:02d}_{model_id}_benchmarks"
        ...
        logger.info(f"Saved: {filename}.json ({analysis['record_count']} records, date={date})")
        return str(filepath)
...
def scrape_api_data(models: List[str] = None, sequences: List[str] = None,
    ...
    print(f"📁 Output: data/raw/json/inferencex/")
    if date:
    ...
    # 保存总结报告
    storage = DataStorage()
    summary_file = storage.save_json(results, 'inferencex', 'api_scraping_summary', timestamp=False)
